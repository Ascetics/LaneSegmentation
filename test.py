...
from utils.augment import PairCrop, PairNormalizeToTensor
from config import Config


def test(net, test_data, device, n_class=8, compare=False):
    """
    测试
    :param net: AI网络
    ...
    with torch.no_grad():  # 测试阶段，不需要计算梯度，节省内存
        for i_batch, (im, lb) in enumerate(test_data):
            ...
            pred = pred.squeeze(0)  # [H,W]tensor
            ...
            if compare:  # 生成对比图
                ...
                miou = get_metrics(cm, metrics='mean_iou')  # 计算本张图像的mIoU
