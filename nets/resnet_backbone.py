...
class ResNetBackBone(nn.Module):
    def __init__(self, block, layers, in_channels=3, batch_norm=None):
        ...
        self.layer4 = self._make_atros(block, layers[3], 512, stride=1, dilation=2)
        pass

    def _make_atros(self, block, n_block, planes, stride=1, dilation=2):
        """
        构造layer4，也就是论文中的Conv5_x，但是不进行下采样，使用Atrous Conv
        :param block: ResNet 18/34 用Basic Block
                      ResNet 50/101/152 用Bottleneck
...
def resnet18_backbone(in_channels=3, batch_norm=None):
    return ResNetBackBone(BasicBlock, [2, 2, 2, 2], in_channels=in_channels,
                          batch_norm=batch_norm)


def resnet34_backbone(in_channels=3, batch_norm=None):
    return ResNetBackBone(BasicBlock, [3, 4, 6, 3], in_channels=in_channels,
                          batch_norm=batch_norm)


def resnet50_backbone(in_channels=3, batch_norm=None):
    return ResNetBackBone(Bottleneck, [3, 4, 6, 3], in_channels=in_channels,
                          batch_norm=batch_norm)


def resnet101_backbone(in_channels=3, batch_norm=None):
    return ResNetBackBone(Bottleneck, [3, 4, 23, 3], in_channels=in_channels,
                          batch_norm=batch_norm)


def resnet152_backbone(in_channels=3, batch_norm=None):
    return ResNetBackBone(Bottleneck, [3, 8, 36, 3], in_channels=in_channels,
                          batch_norm=batch_norm)


if __name__ == '__main__':
    # net = resnet18_backbone()
    # net = resnet34_backbone()
    net = resnet50_backbone()
    # net = resnet101_backbone()
    # net = resnet152_backbone()
    print(net)
