...
class UNetFactory(nn.Module):
    ...
    def __init__(self, encode_blocks, encode_bottom,
                 decode_ups, decode_blocks, n_class, decode_channels=64):
        """
        生成Unet网络
        ...
        self.classifier = nn.Conv2d(decode_channels, n_class, 1)  # 最终输出
        pass
...
def unet_resnet(resnet_type, n_class, upmode='upconv'):
    if resnet_type == 'resnet18':
        resnet = resnet18(pretrained=True)
        encode_blocks = [
            nn.Sequential(),  # output_stride=1x,out_channels=3
            nn.Sequential(resnet.conv1, resnet.bn1, resnet.relu),  # output_stride=2x,out_channels=64
            nn.Sequential(resnet.maxpool, resnet.layer1),  # output_stride=4x,out_channels=64
            resnet.layer2,  # output_stride=8x,out_channels=128
            resnet.layer3,  # output_stride=16x,out_channels=256
        ]
        encode_bottom = resnet.layer4  # output_stride=32x,out_channels=512

        # Upsample
        decode_ups = None  # 上采样可以二选一
        if upmode == 'upconv':  # 转置卷积
            decode_ups = [upconv(512, 256), upconv(256, 128),
                          upconv(128, 64), upconv(64, 32), upconv(3, 3)]
        elif upmode == 'upsample':  # 双线性差值
            decode_ups = [upsample(512, 256), upsample(256, 128),
                          upsample(128, 64), upsample(64, 64), upsample(64, 32)]

        decode_blocks = [
            _UNetDecodeBlock(512, 256, padding=1),  # 第二个decode
            _UNetDecodeBlock(256, 128, padding=1),  # 第三个decode
            _UNetDecodeBlock(128, 64, padding=1),  # 第四个decode
            _UNetDecodeBlock(128, 64, padding=1),
            _UNetDecodeBlock(35, 32, padding=1),
        ]
        return UNetFactory(encode_blocks, encode_bottom, decode_ups, decode_blocks, n_class, 32)
        ...
        encode_blocks = [
            nn.Sequential(),  # output_stride=1x,out_channels=3
            nn.Sequential(resnet.conv1, resnet.bn1, resnet.relu),  # output_stride=2x,out_channels=64
            nn.Sequential(resnet.maxpool, resnet.layer1),  # output_stride=4x,out_channels=256
            resnet.layer2,  # output_stride=8x,out_channels=512
            resnet.layer3,  # output_stride=16x,out_channels=1024
        ]
        encode_bottom = resnet.layer4  # output_stride=32x,out_channels=2048

        # Upsample
        decode_ups = None  # 上采样可以二选一
        if upmode == 'upconv':  # 转置卷积
            decode_ups = [upconv(2048, 1024), upconv(1024, 512),
                          upconv(512, 256), upconv(256, 128), upconv(128, 64)]
        elif upmode == 'upsample':  # 双线性差值
            decode_ups = [upsample(2048, 1024), upsample(1024, 512),
                          upsample(512, 256), upsample(256, 128), upsample(128, 64)]

        decode_blocks = [
            _UNetDecodeBlock(2048, 1024, padding=1),  # 第二个decode
            _UNetDecodeBlock(1024, 512, padding=1),  # 第三个decode
            _UNetDecodeBlock(512, 256, padding=1),  # 第四个decode
            _UNetDecodeBlock(192, 128, padding=1),
            _UNetDecodeBlock(67, 64, padding=1),
        ]
        return UNetFactory(encode_blocks, encode_bottom, decode_ups, decode_blocks, n_class, 64)
